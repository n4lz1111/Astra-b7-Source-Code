...
void TargetHUD::onRenderEvent(RenderEvent& event)
{
    ...
    auto boxSize = ImVec2(240 * anim, 84 * anim);
    ...
    auto headPos = ImVec2(boxPos.x + xpad * anim, boxPos.y - 2 + ypad * anim + 4 * anim);
    ...
    float healthStartY = boxPos.y + boxSize.y - (ypad + 2) * anim - 18 * anim;
    float ysize = 20 * anim;
    ...
    drawList->AddRectFilled(healthBarStart, healthBarEnd, ImColor(100, 100, 100, (int)((float)170 * anim)), 5.f);
    ...
    ImColor startColor = ImColor(0, 255, 0, (int)(255 * anim));
    ImColor endColor = ImColor(70, 255, 70, (int)(255 * anim));
    ...
    if (healthPerc > 0.01f)
    {
        ...
        drawList->AddRectFilledMultiColor(healthBarStart, healthBarEnd, startColor, endColor, endColor, startColor, 5.f, ImDrawCornerFlags_All);
        drawList->PopClipRect();
    }
    ...
    if (absorptionPerc > 0.01f)
    {
        ...
        drawList->AddRectFilled(healthBarStart, healthBarEnd, ImColor(244, 204, 0, (int)(255 * anim)), 5.f);
        drawList->PopClipRect();
    }
