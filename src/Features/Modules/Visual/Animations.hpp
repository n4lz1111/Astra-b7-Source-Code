...
public:
    ...
    EnumSettingT<Animation> mAnimation = EnumSettingT("Animation", "The animation to use", Animation::Blocking, "Default", "Blocking", "1.7");
    ...
    float mSwingAngleValue = 0.f;   // svortl‚ ‚è‚ª‚Æ‚¤
    float* mSwingAngle = &mSwingAngleValue;
    bool mShouldBlock = false;
    float mClientSwingProgress = 0.0f;
