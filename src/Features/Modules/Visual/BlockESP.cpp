...
constexpr int BED = 26;
constexpr int HIVEBED = 10017;
...
std::unordered_map<int, ImColor> blockColors = {
    ...
    { 26, ImColor(1.f, 0.0f, 0.0f, 1.f) },
    { 10017, ImColor(1.f, 0.f, 0.f, 1.f) },
...
std::vector<int> BlockESP::getEnabledBlocks()
{
    ...
    if (mBed.mValue)
    {
        enabledBlocks.push_back(BED);
	}

    if (mHiveBed.mValue)
    {
        enabledBlocks.push_back(HIVEBED);
    }
