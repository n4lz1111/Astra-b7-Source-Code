...
void Fly::onDisable()
{
    ...
    // --- ここからHalt処理を修正 ---
    if (mHalt.mValue) {
        auto& vel = player->getStateVectorComponent()->mVelocity;
        vel.x = 0.f;
        // Yはそのまま
        vel.z = 0.f;
    }
...
void Fly::onBaseTickEvent(BaseTickEvent& event)
{
    ...
    // --- DamageOnlyを全モードで有効にする ---
    if (mDamageOnly.mValue && mLastDamage < NOW) {
        // 速度リセットは不要。もし飛行中のみ止めたい場合は条件を追加してください。
        // mCurrentFriction等のリセットはOK
