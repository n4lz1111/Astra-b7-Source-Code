...
void ModuleManager::init()
{
    ...
    mModules.emplace_back(std::make_shared<Fucker>());
...
void ModuleManager::deserialize(const nlohmann::json& j, bool showMessages)
{
    ...
    std::string currentVersion = Astra_VERSION;

    if (version != currentVersion)
    {
        spdlog::warn("Config version mismatch. Expected: {}, Got: {}", currentVersion, version);
        ChatUtils::displayClientMessage("§eWarning: The specified config is from a different version of Astra. §cSome settings may not be loaded§e.");
    }
