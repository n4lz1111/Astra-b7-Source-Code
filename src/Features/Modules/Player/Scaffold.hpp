...
public:
    ...
    NumberSetting mPlaces = NumberSetting("Places", "The amount of blocks to place per tick", 1.25, 0, 20, 0.05);
    NumberSetting mRange = NumberSetting("Range", "The range at which to place blocks", 7.75, 0, 10, 0.01);
    NumberSetting mExtend = NumberSetting("Extend", "The distance to extend the placement", 1, 0, 10, 1);
    EnumSettingT<RotateMode> mRotateMode = EnumSettingT<RotateMode>("Rotate Mode", "The mode of rotation", RotateMode::Backwards, "None", "Normal", "Down", "Backwards");
    ...
    EnumSettingT<SwitchMode> mSwitchMode = EnumSettingT<SwitchMode>("Switch Mode", "The mode for block switching", SwitchMode::Spoof, "None", "Full", "Fake", "Spoof");
    EnumSettingT<SwitchPriority> mSwitchPriority = EnumSettingT<SwitchPriority>("Switch Prio", "The priority for block switching", SwitchPriority::Highest, "First", "Highest");
    BoolSetting mHotbarOnly = BoolSetting("Hotbar Only", "Whether or not to only place blocks from the hotbar", true);
    EnumSettingT<TowerMode> mTowerMode = EnumSettingT<TowerMode>("Tower Mode", "The mode for tower placement", TowerMode::Velocity, "Vanilla", "Velocity", "Clip", "Timer");
    NumberSetting mTowerSpeed = NumberSetting("Tower Speed", "The speed for tower placement", 3.004, 0, 20, 0.01);
    ...
    BoolSetting mAvoidUnderplace = BoolSetting("Avoid Underplace", "Whether or not to avoid underplacing", true);
    ...
    BoolSetting mLockY = BoolSetting("Lock Y", "Whether or not to lock the Y position", true);
    BoolSetting mSwing = BoolSetting("Swing", "Whether or not to swing the arm", false);
    BoolSetting mTest = BoolSetting("Diagonal bypass", "FlareonV1", false);
    BoolSetting mDelay = BoolSetting("Place Delay", "Whether or not to use Delay", true);
    NumberSetting mDelayValue = NumberSetting("Delay Value", "Delays at places", 30, 0, 360, 10);
    BoolSetting mFirstEvent = BoolSetting("First Event", "Patch for very standard way to detect scaffold", false);
    EnumSettingT<ClickPosition> mClickPosition = EnumSettingT<ClickPosition>("Click Position", "The mode of click position", ClickPosition::Bedrock, "Normal", "Bedrock");
    ...
    Scaffold() : ModuleBase("Scaffold", "Automatically places blocks below you", ModuleCategory::Player, 0, false) {
        ...
        VISIBILITY_CONDITION(mDelayValue, mDelay.mValue)
    ...
    bool isPlacing() const {
        return mShouldRotate && mRotateMode.mValue != RotateMode::None;
    }
    uint64_t mLastSwitchTime = 0;
    int mLastSlot = -1;
    bool mShouldClip = false;
    glm::vec2 mLastRot = { 0.f, 0.f };


    // Tower stuff
    bool mIsTowering = false;
    float mPlaceAccumulator = 0.0f;
