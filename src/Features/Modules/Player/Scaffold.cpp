...
void Scaffold::onBaseTickEvent(BaseTickEvent& event)
{
    ...
    // === ブロック設置数決定 ===
    float addPlaces = mPlaces.as<float>();
    if (mFastClutch.mValue && player->getFallDistance() > mClutchFallDistance.as<float>())
        addPlaces = mCluchPlaces.as<float>();

    // Tower判定（WASD押してない＝横移動なし）
    auto input = player->getMoveInputComponent();
    bool towering = false;
    if (input)
        towering = !input->mForward && !input->mRight && !input->mLeft && !input->mBackward;

    // Tower中 or OnGroundなら Delay 半分
    float delayMs = mDelayValue.as<float>();
    if (towering || player->isOnGround())
        delayMs *= 0.5f;

    // 累積に加算
    mPlaceAccumulator += addPlaces;
    int placeCount = static_cast<int>(mPlaceAccumulator);

    if (placeCount == 0) return; // 置くブロックがない
    ...
    for (int i = 0; i < placeCount; ++i) {
        ...
        if (mDelay.mValue) {
            ...
            if (now - mLastPlaceTime < static_cast<uint64_t>(delayMs)) {
                // まだ待機時間が足りない → このtickでは置かない
                break;
            }
