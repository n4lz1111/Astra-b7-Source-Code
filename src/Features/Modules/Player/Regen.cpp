...
#include <SDK/Minecraft/Network/MinecraftPackets.hpp>       // createPacket, Action
#include <SDK/Minecraft/World/BlockLegacy.hpp>
#include <SDK/Minecraft/World/Level.hpp>
#include <SDK/Minecraft/World/HitResult.hpp>
//#include <SDK/Minecraft/ClientInstance.hpp>
//#include <SDK/Minecraft/LocalPlayer.hpp>
...
void Regen::onBaseTickEvent(BaseTickEvent& event) {
    ...
    // =======================
    // MiningBypassMode: StopInAir
    // =======================
    if (mMiningBypassMode.mValue == MiningBypassMode::Eclipse) {
        if (!localplayer->isOnGround()) {
            // 空中では掘らない
            if (mIsMiningBlock) {
                //mIsMiningBlock = false;
                mBreakingProgress = false -0.02f; // プログレスバーもリセット
            }
        }
        else {
            // 地面に戻ったら掘り再開
            if (!mIsMiningBlock && mCurrentBlockPos != glm::ivec3{ INT_MAX, INT_MAX, INT_MAX }) {
                //mIsMiningBlock = true;
                mBreakingProgress = true; // プログレスバー再スタート
            }
        }
    }

    if (mMiningBypassMode.mValue == MiningBypassMode::Astra) {
        if (!localplayer->isOnGround()) {
            // 空中では掘らない
            if (mIsMiningBlock) {
                //mIsMiningBlock = false;
                mBreakingProgress = - 0.02f; // プログレスバーもリセット
            }
        }
        else {
            // 地面に戻ったら掘り再開
            if (!mIsMiningBlock && mCurrentBlockPos != glm::ivec3{ INT_MAX, INT_MAX, INT_MAX }) {
                //mIsMiningBlock = true;
                mBreakingProgress = 0.02f; // プログレスバー再スタート
            }
        }
    }
    ...
    float coveredBlockProgress = coveredBlockDestroySpeed * (coveredBlockBreakingTime / static_cast<float>(50));
    ...
    if (
        ...
        switch (mRotationType.mValue) {
        case RotationType::Old:
            if (mBreakingProgress >= mCurrentDestroySpeed)
                mShouldRotate = true;
            break;

        case RotationType::Always:
            if (mBreakingProgress < mCurrentDestroySpeed)
                mShouldRotate = true;
            else
                mShouldRotate = false;
        ...
        case RotationType::Percent:
            if (mBreakingProgress >= mCurrentDestroySpeed * mPercent.mValue)
                mShouldRotate = true;
            else
                mShouldRotate = false;
            break;
        ...
        if (player->isOnGround()) {
            ...
            if (mSwingMode.mValue == SwingMode::Normal) {
                player->swing(); // クライアント側アニメ
            }
            else if (mSwingMode.mValue == SwingMode::Silent) {
                // Silent: 掘ってる間サーバーにもSwingを通知
        ...
        if ((mCurrentDestroySpeed <= mBreakingProgress && (!mIsStealing || exposedFace != -1)) && finishBreak) {
            //mShouldRotate = true;        Old Rotation
        if (mSwingMode.mValue == SwingMode::OldPacket) {
            // OldPacket: 破壊完了時のみ
            if (finishBreak) {
                auto pkt = MinecraftPackets::createPacket<AnimatePacket>();
                pkt->mRuntimeID = player->getRuntimeID();
        ...
        else if (mSwingMode.mValue == SwingMode::Old) {
            // Old: 破壊完了時のみ
            if (finishBreak)
                player->swing();
        }
...
void Regen::onPacketOutEvent(PacketOutEvent& event)
{
    ...
    if (event.mPacket->getId() == PacketID::PlayerAuthInput)
    {
        ...
        if (mShouldRotate) {
            const auto blockAABB = AABB(mCurrentBlockPos, { 1,1,1 });
            const auto rotations = MathUtils::getRots(*player->getPos(), blockAABB);
            ...
            if (auto headComp = player->getActorHeadRotationComponent()) {
                headComp->mHeadRot = rotations.y;
            }

            if (mRotationType.mValue == RotationType::Old)
                mShouldRotate = false;

            if (mRotationType.mValue == RotationType::Percent)
                mShouldRotate = false;

            if (mRotationType.mValue == RotationType::Always)
                mShouldRotate = false;
        }
