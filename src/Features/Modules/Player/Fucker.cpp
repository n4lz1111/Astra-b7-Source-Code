...
bool Fucker::isValidBlock(glm::ivec3 blockPos, bool oreOnly, bool exposedOnly, bool usePriority, OrePriority priority) {
    auto player = ClientInstance::get()->getLocalPlayer();
...
void Fucker::queueBlock(glm::ivec3 blockPos) {
    Block* block = ClientInstance::get()->getBlockSource()->getBlock(blockPos);
...
Fucker::PathFindResult Fucker::getBestPathToBlock(glm::ivec3 blockPos) {
    auto player = ClientInstance::get()->getLocalPlayer();
...
bool Fucker::isOre(std::vector<int> Ids, int id)
{
    return std::find(Ids.begin(), Ids.end(), id) != Ids.end();
}

void Fucker::onEnable()
{
    gFeatureManager->mDispatcher->listen<BaseTickEvent, &Fucker::onBaseTickEvent>(this);
    gFeatureManager->mDispatcher->listen<PacketOutEvent, &Fucker::onPacketOutEvent, nes::event_priority::VERY_LAST>(this);
    gFeatureManager->mDispatcher->listen<RenderEvent, &Fucker::onRenderEvent>(this);
...
{
    gFeatureManager->mDispatcher->deafen<BaseTickEvent, &Fucker::onBaseTickEvent>(this);
    gFeatureManager->mDispatcher->deafen<PacketOutEvent, &Fucker::onPacketOutEvent>(this);
    gFeatureManager->mDispatcher->deafen<RenderEvent, &Fucker::onRenderEvent>(this);
...
void Fucker::onBaseTickEvent(BaseTickEvent& event)
{
...
void Fucker::onRenderEvent(RenderEvent& event)
{
    auto player = ClientInstance::get()->getLocalPlayer();
    if (!player) return;
...
void Fucker::onPacketOutEvent(PacketOutEvent& event)
{
    auto player = ClientInstance::get()->getLocalPlayer();
    if (!player) return;
