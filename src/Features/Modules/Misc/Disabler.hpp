...
public:
    ...
    EnumSettingT<Mode> mMode = EnumSettingT<Mode>("Mode", "The mode to use for the disabler.", Mode::Flareon, "Flareon", "Sentinel", "Lifeboat",
        ...
        "SentinelNew",
#endif
        "Custom", "BDS Prediction", "sentinel full ", "Nekozo");
    ...
    BoolSetting mSwingPackets = BoolSetting("Swing Packets", "Send swing packets without client animation", false);
    BoolSetting mSprintPackets = BoolSetting("Sprint Packets", "Send sprint packets even when not sprinting", false);
	BoolSetting mCiv = BoolSetting("Civ", "Sends fake end stone packets to bypass civ break disabler", false);
	BoolSetting mJavaFly = BoolSetting("Java Fly", "Java Vanilla Fly kick disabled", true);
	BoolSetting mJEMoveFix = BoolSetting("JE Move Fix", "(test)Attempt to fix movement issues on Java Edition servers", true);
    ...
    Disabler() : ModuleBase<Disabler>("Disabler", "Attempts to disable Anti-Cheat checks by exploiting them.", ModuleCategory::Misc, 0, false) {
        ...
        addSetting(&mSwingPackets);
		addSetting(&mSprintPackets);
		addSetting(&mCiv);
		addSetting(&mJavaFly);
		addSetting(&mJEMoveFix);
        ...
        VISIBILITY_CONDITION(mSwingPackets, mMode.mValue == Mode::Flareon);
        VISIBILITY_CONDITION(mSprintPackets, mMode.mValue == Mode::Flareon);
		...
		VISIBILITY_CONDITION(mCiv, mMode.mValue == Mode::Nekozo);
        VISIBILITY_CONDITION(mJavaFly, mMode.mValue == Mode::Nekozo);
        VISIBILITY_CONDITION(mJEMoveFix, mMode.mValue == Mode::Nekozo);
