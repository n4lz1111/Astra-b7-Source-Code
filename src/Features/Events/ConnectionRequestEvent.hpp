...
public:
    ...
    explicit ConnectionRequestEvent(
        class ConnectionRequest* result,
        class PrivateKeyManager* privKeyManager,
        class Certificate* cert,
        std::string* selfSignedId,
        std::string* serverAddress,
        int64_t clientRandomId,
        std::string* skinId,
        const char* skinData,
        const char* capeData,
        class SerializedSkin* skin,
        std::string* deviceId,
        int inputMode,
        int uiProfile,
        int guiScale,
        ...
        : mResult(result),
        mPrivKeyManager(privKeyManager),
        mCert(cert),
        mSelfSignedId(selfSignedId),
        mServerAddress(serverAddress),
        mClientRandomId(clientRandomId),
        mSkinId(skinId),
        mSkinData(skinData),
        mCapeData(capeData),
        mSkin(skin),
        ...
        mIsEduMode(IsEduMode),
        mTenantId(tenantId),
        mAdRole(adRole),
        mPlatformUserId(platformUserId),
        mThirdPartyName(thirdPartyName),
        mThirdPartyNameOnly(thirdPartyNameOnly),
        mPlatformOnlineID(platformOnlineID),
        mPlatformOfflineID(platformOfflineID),
        mCapeId(capeId),
        mCompatibleWithClientSideChunkGen(compatibleWithClientSideChunkGen)
    {
    }
};
